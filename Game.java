...
public class Game { // движок игры
    ...
    public void initTable() {
        ...
        String column[]={"Общее время (c)","Уровень сложности","Время","Результат"};
        frame.setBounds(30,40,500,300);
    ...
    private void level(){
        if (bomb.getTotalBombs() == 8)
            level = EASY;
        if (bomb.getTotalBombs() == 16)
            level = MEDIUM;
        if (bomb.getTotalBombs() == 32)
            level = HARD;
    }
